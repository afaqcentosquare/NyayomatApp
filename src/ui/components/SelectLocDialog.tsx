...
type Props = {
    ...
    closeDialog : ()  => void,
    selectLocTxt : (txt : string) => void,
    locDialogList : locObj[],
    locDataProgress : boolean,
    locDataTxt : boolean
}

export const SelectLocDialog = React.memo<Props>((props) =>
{
    const countryTxt = strings.countryListTitle;

    const locListItem = (item : locObj) =>
    {
        return(
            <TouchableOpacity
                onPress={() => {
                    props.selectLocTxt( item.name)
                    props.closeDialog()
                }}
        ...
        <Modal
            ...
            <View style={styles.dialogMainContainer}>
                <View style={styles.dialogSubContainer}>
                    ...
                    {props.locDialogList !== null ? <View style={{width: Dimensions.get('window').width / 1.2}}>
                        <FlatList
                            style={{marginTop: 20}}
                            data={props.locDialogList}
                            showsHorizontalScrollIndicator={false}
                            showsVerticalScrollIndicator={false}
                            renderItem={({item, index}) => locListItem(item)}
                            keyExtractor={(item, index) => index.toString()}/>
                    </View> : null}
                    {props.locDataProgress ? <View style={{flex:1,justifyContent:'center',width: Dimensions.get('window').width / 1.2 }}>
                        <ProgressBar/>
                    </View> : null}
                    {props.locDataTxt ? <View style={{flex:1,width: Dimensions.get('window').width / 1.2 }}>
                        <NoDataTxt/>
                    </View> : null}
                </View>
            </View>
